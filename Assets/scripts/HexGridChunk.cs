 ...
 *
 * Description:
 * Handles the low-level mesh generation for hexagonal map chunks,
 * including terrain, rivers, roads, and water surfaces. Each chunk
 * represents a distinct section of the world and is responsible for
 * triangulating its contained HexCells into renderable geometry.
 *
 * This implementation extends Jasper Flick’s foundational work with:
 *  • Multiplayer synchronization via SharedHexGridChunk integration
 *  • Water-level and terrain updates propagated through HexServer
 *  • Feature and special structure placement managed by HexFeatureManager
 *
 * Note:
 * The geometric triangulation and mesh construction logic are derived
 * directly from the original tutorial. All networking and data-linking
 * extensions are original work.
 */
...
public class HexGridChunk : MonoBehaviour {
    ...
    public SharedHexGridChunk sharedChunk;
	...
	public void LinkCells(SharedTileInfo sharedTileInfo, int index)
	{
		cells[index].sharedTileInfo = sharedTileInfo;
		sharedTileInfo.hexCell = cells[index];
	}

	public void RaiseWaterLevel(int level)
	{
		foreach (HexCell cell in cells)
		{
			if (cell.sharedTileInfo.IsOwner)
			{
				if (cell.WaterLevel < level)
				{
